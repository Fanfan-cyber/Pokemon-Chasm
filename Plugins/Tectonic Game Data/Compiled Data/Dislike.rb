...
module GameData
    ...
    def self.getAllDislike
      dislikes = DATA.values
      dislikes.reject! { |dislike| dislike.id.start_with?("DEBUG_") }
      return dislikes.uniq
    end

    def self.getAllDislikeNames
      getAllDislike.map(&:name)
    end
	end
end
