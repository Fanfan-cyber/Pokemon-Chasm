...
class Pokemon
    ...
    def gender=(value)
        @gender = value if value.nil?
        return if singleGendered?
        @gender = value if value == 0 || value == 1
    end
...
=end
    ...
    def choose_trait(which = 0)
        ...
        pbMessage(_INTL("This Trait has been set to {1}!", names[choose]))
    end
    ...
    def choose_like
        likes = GameData::Like.getAllLike
        names = likes.map(&:name)
        names << _INTL("Cancel")
        choose = pbMessage(_INTL("Which do you want?"), names, -1)
        return if choose == -1 || choose == names.size - 1
        @Like = likes[choose]
        pbMessage(_INTL("The Like has been set to {1}!", names[choose]))
    end
    ...
    def choose_dislike
        dislikes = GameData::Dislike.getAllDislike
        names = dislikes.map(&:name)
        names << _INTL("Cancel")
        choose = pbMessage(_INTL("Which do you want?"), names, -1)
        return if choose == -1 || choose == names.size - 1
        @Dislike = dislikes[choose]
        pbMessage(_INTL("The Dislike has been set to {1}!", names[choose]))
    end
