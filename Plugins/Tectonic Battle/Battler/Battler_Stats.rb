class PokeBattle_Battler
    ...
    def pbDefense(aiCheck = false, step = nil)
        ...
        # Hail and Ice Age
        if @battle.icy? && (pbHasType?(:ICE) || (pbHasType?(:GHOST) && @battle.pbWeather == :IceAge) || @battle.curseActive?(:CURSE_NO_MOVING_CYCLICAL) && !pbOwnedByPlayer?)
            hailAddition = 0.5
    ...
    def pbSpeed(aiCheck = false, step = nil, afterSwitching: false, move: nil)
        ...
        unless afterSwitching
            speedMult *= 1.33 if tailWind?
            speedMult *= 0.67 if headWind?
        ...
        if numbRelevant
            speedMult /= 2.0
            speedMult /= Settings::CURSE_STATUS_DOUBLED_RATE if pbOwnedByPlayer? && @battle.curseActive?(:CURSE_STATUS_DOUBLED)
    ...
    def headWind?
        return true if pbOpposingSide.effectActive?(:Tailwind)
        return true if pbOpposingSide.effectActive?(:EmpoweredTailwind)
        return false
    end
