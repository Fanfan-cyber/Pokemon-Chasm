...
class PokeBattle_Battler
    ...
    def abilityActive?(ignore_fainted = false, ignore_gas = false)
        ...
        return false if !ignore_gas && @battle.abilitiesNeutralized? && !abilities.include?(:NEUTRALIZINGGAS)
    ...
    def healthCapped?
        return @hp >= @totalhp * MAX_CAP_RATIO if forceOverheal?
        return fullHealth?
    end
    ...
    def canGulpMissile?
        return @species == :CRAMORANT && hasActiveAbility?([:GULPMISSILE, :SWALLOWMISSILE]) && @form.zero?
    end
