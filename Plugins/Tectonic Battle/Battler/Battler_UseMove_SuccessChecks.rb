class PokeBattle_Battler
    ...
    def tortured?
        return true if effectActive?(:Torture)
        return true if pbOwnSide.effectActive?(:TurbulentSky)
        return false
    end
        ...
        if pbOwnedByPlayer?
            ...
            if @battle.tracker_get(:alt_dimension_d).include?(@pokemon.unique_id)
                if aiCheck
                    ...
                    @battle.pbDisplay(_INTL("{1} cannot battle due to the Species Clause!", pbThis))
                    onMoveFailed(move)
                    return false
                end
            end
