...
class PokeBattle_Battler
    ...
    def pbLowerStatStep(stat, increment, user = nil, showAnim = true, ignoreContrary = false, ignoreMirrorArmor = false, ignoreStubborn = false)
        ...
        if hasActiveAbility?(:STUBBORN) && !ignoreStubborn && !@battle.moldBreaker && increment > 1
            ...
            if user && user.index != self.index && user.takesIndirectDamage?
    ...
    def pbResetRaisedStatSteps(showMessage = false, aiCheck = false)
        anyReset = false
        score = 0
        GameData::Stat.each_battle { |s|
            stat = s.id
            statStep = @steps[stat]
            next unless statStep > 0

            if aiCheck
                if stat == :ATTACK
                    next unless hasPhysicalAttack?
                elsif stat == :SPECIAL_ATTACK
                    next unless hasSpecialAttack?
                end
                score += statStep * 30
    ...
    def pbReverseStatSteps(showMessage = false)
        anyReset = false
    ...
    alias invertStatSteps pbReverseStatSteps
end
