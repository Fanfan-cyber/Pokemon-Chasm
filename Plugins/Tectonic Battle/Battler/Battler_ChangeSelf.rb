...
class PokeBattle_Battler
    ...
    def can_faint_healing?
        ...
        if !Settings::PKMN_REVIVAL || !@battle.trainerBattle? || !TA.get(:battlerevive) || battle_tracker_get(:faint_healing_triggered)
            if !TA.get(:casualmode) && pbOwnedByPlayer? && (isSpecies?(:GARDEVOIR) && !$Trainer.has_species?(:GALLADE) ||
    ...
    def apply_faint_healing
        ...
        healing_turn = @battle.singleBattle? ? (TA.get(:customrevivalturnsingle) || Settings::FAINT_HEALING_TURN) : (TA.get(:customrevivalturndouble) || Settings::FAINT_HEALING_TURN_DOULBE)
        @battle.pbDisplay(_INTL("{1} will revive in {2} turns!", pbThis, healing_turn))
...
=end
        ...
        new_ability = old_abilities.concat(target.abilities)
...
=end
        add_random_ability(false, false) if hasLocket
        add_random_ability(false, false) if @battle.curseActive?(:CURSE_DOUBLE_ABILITIES) && index.odd?
    ...
    def replaceAbility(newAbility, showSplashes = true, swapper = nil, replacementMsg: nil)
        new_abilities = newAbility.is_a?(Array) ? newAbility.clone : [newAbility]
        return if new_abilities.empty? || new_abilities.all? { |ability| hasAbility?(ability) }
        @battle.pbShowAbilitySplash(swapper, new_abilities.first) if showSplashes && swapper
        old_abilities = abilities.clone
        lost_abilities = old_abilities - new_abilities
        showMyAbilitySplash(firstAbility, true) if showSplashes
        setAbility(new_abilities)
        @battle.pbReplaceAbilitySplash(self, new_abilities.first) if showSplashes
        new_abilities_names = new_abilities.map { |ability| getAbilityName(ability) }
        replacementMsg ||= _INTL("{1}'s Ability became {2}!", pbThis, new_abilities_names.quick_join)
        ...
        pbOnAbilitiesLost(lost_abilities) unless lost_abilities.empty?
        pbEffectsOnSwitchIn
    end
