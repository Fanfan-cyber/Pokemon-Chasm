...
immuneTypeRealName))
        ...
        # Status Clause
        if Settings::STATUS_CLAUSE && !statusDoublingCurse && owned_trainer&.should_apply_status_clause?(newStatus)
            @battle.pbDisplay(_INTL("It doesn't affect {1} because two Pokémon in the party are already under this status!", pbThis(true))) if showMessages
            return false
        end
    ...
    def canSleepYawn?
        ...
        # Status Clause
        statusDoublingCurse = pbOwnedByPlayer? && @battle.curseActive?(:CURSE_STATUS_DOUBLED)
        return false if !statusDoublingCurse && Settings::STATUS_CLAUSE && owned_trainer&.should_apply_status_clause?(:SLEEP)
