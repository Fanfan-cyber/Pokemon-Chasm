class PokeBattle_Battle
    ...
    def pbStartBattle
        ...
        unless @autoTesting
            ...
            if @decision == 1
                if trainerBattle? && !TA.get(:battle_loader) && !@is_replayed #&& !skipPerfecting
                    if (TA.get(:ableAfterFight) || $Trainer.able_pokemon_count) >= ableBeforeFight || TA.get(:ignore_perfect)
                        ...
                        if TA.get(:win, 0).zero? # First time win
                            pbMessage(_INTL("<imp>Wow, you've finally defeated the formidable opponent!\nPlease keep it up, don't give up!</imp>"))
                            pbReceiveItem(:EXPCANDYXL) # Reward
                        else
                            #pbMessage(_INTL("<imp>{1}</imp>", GOOD_BUT_COOL.sample)) if is_chinese?
                        end
                        TA.increase(:win)
                    end
    ...
    def pbStartBattleCore(ableBeforeFight = nil)
        ...
        eachOtherSideBattler { |b| b.add_random_ability(false, false) } if curseActive?(:CURSE_DOUBLE_ABILITIES)
        ...
        pbDisplay(_INTL("You have to win the battle <c3=E82010,F8A8B8>PERFECTLY</c3>, or you LOSE!")) if trainerBattle? && !TA.get(:disableperfect) #&& !TA.get(:battle_loader)
        ...
        when 2, 5
            ...
            if @internalBattle
                if trainerBattle?
                    ...
                    unless TA.get(:battle_loader) || @is_replayed
                        if TA.get(:lose, 0).zero? # First time lose
                            pbMessage(_INTL("<imp>Hang in there! You're doing great.\nPlease keep it up, don't give up!</imp>"))
                            pbReceiveItem(:RARECANDY) # Reward
                        else
                            #pbMessage(_INTL("<imp>{1}</imp>", BAD_BUT_COOL.sample)) if is_chinese?
                        end
                        TA.increase(:lose) 
                    end
                end
