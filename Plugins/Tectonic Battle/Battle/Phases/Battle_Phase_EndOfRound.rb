...
=end
        ...
        no_darkscale_clouds = []
        priority.each do |b|
            if b.hasActiveAbility?(:DARKSCALECLOUD)
                darkscale_clouds << b
            else
                no_darkscale_clouds << b
            end
        end
        if darkscale_clouds.any?
            no_darkscale_clouds.each do |b|
                if b.tracker_get(:darkscale_cloud_turn) < 1
                    b.tracker_increment(:darkscale_cloud_turn)
                elsif b.canPoison?(nil, false)
                    pbDisplay(_INTL("{1} inhaled too much toxic dust!", b.pbThis))
                    b.applyPoison(nil)
                    darkscale_clouds.each { |ba| ba.tryRaiseStat(:DEFENSE, ba, ability: :DARKSCALECLOUD, increment: 2) }
                end
            end
        end
        ...
        [@party1, @party2].each_with_index do |pty, i|
            pty.each do |pkmn|
                ...
                if i == 0
                  ...
                  next if pkmn.pinch? && !TA.get(:opposingalwaysresting)
                end
    ...
    def damageFromDOTStatus(battler, status, aiCheck = false)
        if battler.takesIndirectDamage? && !battler.hasActiveAbility?(:PLACIDITY)
            ...
            fraction *= Settings::CURSE_STATUS_DOUBLED_RATE if battler.pbOwnedByPlayer? && curseActive?(:CURSE_STATUS_DOUBLED)
        ...
        battlersInOrder.each do |b|
            ...
            if damageDealt > 0
                priority.each do |tax_user|
                    ...
                    tax_user.pbRecoverHP(damageDealt * 1.5, true, true, true, healingMessage)
