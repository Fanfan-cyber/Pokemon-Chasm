class PokeBattle_Battle
    ...
    def forceUseMove(forcedMoveUser, moveIDOrIndex, target = -1, specialUsage = true, usageMessage = nil, moveUsageEffect: nil, ability: nil, forceBaseDamage: nil, aiCheck: false)
        ...
        unless cantForceMove
            PBDebug.logonerr do
                forcedMoveUser.applyEffect(:ForceBaseDamage, forceBaseDamage) if forceBaseDamage
    ...
    def aiLearnsAbility(battler, ability)
        return # don't need to learn anymore
    ...
    def aiSeesMove(battler, moveID)
        return # don't need to see anymore
        return unless battler.pbOwnedByPlayer?
    ...
    def aiKnownMoves(pokemon) # unused
    ...
    def predictable?(predictor,idxBattler)
        return false unless predictor.ownersPolicies.include?(:PREDICTS_PLAYER) || Settings::AI_PREDICT && !TA.get(:stupidai) # always predicts
