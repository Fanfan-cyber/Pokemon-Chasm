...
BattleHandlers::LoadDataDependentAbilityHandlers += proc {
  GameData::Ability.getByFlag("Levitation").each do |abilityID|
      BattleHandlers::AbilityOnSwitchIn.add(abilityID,
          proc { |ability, battler, battle, aiCheck|
              next 0 if aiCheck
              battle.pbShowAbilitySplash(battler, ability)
...
BattleHandlers::AbilityOnSwitchIn.add(:IMPOSTER,
  proc { |ability, battler, battle, aiCheck|
      ...
      next 0 if choice.pokemon.abilities.any? { |abil| GameData::Ability.getByFlag("UnableByDefault").include?(abil) }
      next 40 if aiCheck
      battle.pbShowAbilitySplash(battler, ability, true)
      battle.pbHideAbilitySplash(battler)
...
BattleHandlers::AbilityOnSwitchIn.add(:RAINBOWTRAIL,
  proc { |ability, battler, battle, aiCheck| 
      next unless battler.canChangeType?
      next unless battler.pbHasType?(:FIRE)
      next 0 if aiCheck
...
BattleHandlers::AbilityOnSwitchIn.add(:ADRENALINERUSH,
  proc { |ability, battler, battle, aiCheck|
      next 0 if aiCheck
      battle.pbShowAbilitySplash(battler, ability)
      battle.pbDisplay(_INTL("{1}'s adrenaline is on the rise!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
  }
)