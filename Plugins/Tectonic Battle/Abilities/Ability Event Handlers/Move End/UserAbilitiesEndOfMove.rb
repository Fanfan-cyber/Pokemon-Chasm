...
BattleHandlers::UserAbilityEndOfMove.add(:OVERCHARGE,
  proc { |ability, user, targets, _move, battle, _switchedBattlers|
      ...
      targets.each { |b| numFainted += 1 if b.damageState.fainted }
      next if numFainted == 0
      user.pbRaiseMultipleStatSteps([:SPECIAL_ATTACK, numFainted * 2, :SPEED, numFainted * 2], user, ability: ability)
  }
)
...
BattleHandlers::UserAbilityEndOfMove.add(:AEROSHELL2,
...
BattleHandlers::UserAbilityEndOfMove.copy(:FRIGHTENINGFANGS, :FRIGHTENINGBITE)
...
BattleHandlers::UserAbilityEndOfMove.add(:RELENTLESSPURSUIT,
  proc { |ability, user, targets, _move, _battle, _switchedBattlers|
      next unless user.effectActive?(:RelentlessPursuit)
      next unless user.canHeal?(true)
      totalDamageDealt = 0
      targets.each do |target|
          next if target.damageState.unaffected
          totalDamageDealt += target.damageState.totalHPLost
      ...
      next if totalDamageDealt <= 0
      healingMessage = _INTL("{1} is gloating.", user.pbThis)
      user.pbRecoverHP(totalDamageDealt, true, true, true, healingMessage, canOverheal: true)
  }
)