...
class PokeBattle_Move_SetUserAbilityToTargetAbility < PokeBattle_Move
    ...
    def pbFailsAgainstTarget?(user, target, show_message)
        ...
        if user.abilities.sort == target.abilities.sort
            if show_message
    ...
    def pbEffectAgainstTarget(user, target)
        replacementMsg = _INTL("{1} copied {2}'s all abilities!", user.pbThis, target.pbThis(true), getAbilityName(target.firstAbility))
        #user.replaceAbility(target.firstAbility, replacementMsg: replacementMsg)
        user.replaceAbility(target.abilities, replacementMsg: replacementMsg)
    end
...
class PokeBattle_Move_SetTargetAbilityToUserAbility < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user, target)
        replacementMsg = _INTL("{1} copied {2}'s all abilities!", target.pbThis, user.pbThis(true), getAbilityName(target.firstAbility))
        target.replaceAbility(user.abilities, replacementMsg: replacementMsg)
    end
