...
=end
      ...
      if !ball.legal?
        pbMessage(_INTL("\\l[4]Pokemon {1} in {2} is in a {3}. That ball has been cut from the game or is not legal to own. Swapping now.", name, location, ball.name)) unless skipLegalityMessages? || source
        pokemon.poke_ball = :POKEBALL
        anyPokemonChanged = true
      end

      if pokemon.species == :SHEDINJA && pokemon.gender != 2
        pbMessage(_INTL("\\l[4]Pokemon {1} in {2} has the wrong gender. That is not legal for it. Swapping now.", name, location)) unless skipLegalityMessages? || source
        pokemon.gender = nil
        anyPokemonChanged = true
      end
