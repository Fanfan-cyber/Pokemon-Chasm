class PokeBattle_Battle
  ...
  def player_most_type
    @player_most_type ||= calculate_most_type(@party1)
  end

  def opposing_most_type
    @opposing_most_type ||= calculate_most_type(@party2)
  end

  def calculate_most_type(party)
    type_count = Hash.new(0)
    party.each do |pokemon|
      next unless pokemon
      types = pokemon.hasItem?(:CRYSTALVEIL) ? [pokemon.itemTypeChosen] : pokemon.types
      types.each { |type| type_count[type] += 1 }
    end
    return [[], 0] if type_count.empty?

    max_count = type_count.values.max
    max_types = type_count.select { |_, count| count == max_count }.keys
    [max_types, max_count]
  end
