class Pokemon
  ...
  def hasMultipleItemAbility?
    abilities.any? { |abil_id| GameData::Ability.get(abil_id).is_multiple_item_ability? && abil_id != :STICKYFINGERS } # may need to optimize :STICKYFINGERS
  end
end
...
module AbilityRecorder
  ...
  def self.oppen_ability_recorder(pkmn)
    check_legality
    abils = $Trainer.ability_recorder
    chose = change_ability_choose_from_list(pkmn, abils.uniq, true) do |abil_id, abil_name|
