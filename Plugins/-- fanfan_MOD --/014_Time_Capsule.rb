module TimeCapsule
  ...
  def self.open_time_capsule
    unless $Trainer.has_pokemon? && teamEditingAllowed?
      pbMessage(_INTL("You can't open the Time Capsule now!"))
      return
    end
    loop do
      choice = [_INTL("Retrieve"), _INTL("Explain"), _INTL("Cancel")]
      choose = pbMessage(_INTL("What do you want to do?"), choice, -1)
      case choose
      when -1, 2 # Cancel
        break
      when 1 # Explain
        pbMessage(_INTL("Time Capsule is a place where you can store Pokémon.\nPokémon in the Time Capsule are accessible across all save files."))
      when 0 # Retrieve
  ...
  def self.add_to_time_capsule(pkmn, ignore_confirm = false)
    if ignore_confirm || pbConfirmMessage(_INTL("Do you want to put {1} in the Time Capsule?", pkmn.name))
      pbTakeItemsFromPokemon(pkmn) if pkmn.hasItem?
      @@time_capsule << pkmn
