    ...
    priority.each do |b|
      ...
      step_counter.each do |stat, counters|
        ...
        counters.each do |counter| # [increment1, remaining1]
          next if b.skip_tick_down?(stat, counter[0])
          counter[1] -= 1
          next unless counter[1] == 0
          recoverable = counter[0] < 0 && !b.statStepAtMax?(stat) ||
                        counter[0] > 0 && !b.statStepAtMin?(stat)
          if recoverable
            ideal_steps = b.steps[stat] - counter[0] # e.g -5 - 5 = -10 or 5 - (-5) = 10
            out_of_bounds = false
            if ideal_steps < -PokeBattle_Battler::STAT_STEP_BOUND # -10 < -8
              real_steps = -PokeBattle_Battler::STAT_STEP_BOUND
              out_of_bounds = -PokeBattle_Battler::STAT_STEP_BOUND - ideal_steps # -8 - (-10) = 2
              count_steps = b.steps[stat] + PokeBattle_Battler::STAT_STEP_BOUND  # -5 + 8 = 3
            elsif ideal_steps > PokeBattle_Battler::STAT_STEP_BOUND # 10 > 8
              real_steps = PokeBattle_Battler::STAT_STEP_BOUND
              out_of_bounds = PokeBattle_Battler::STAT_STEP_BOUND - ideal_steps # 8 - 10 = -2
              count_steps = b.steps[stat] - PokeBattle_Battler::STAT_STEP_BOUND # 5 - 8 = -3
            end
            if out_of_bounds
              b.steps[stat] = real_steps
              stat_changes[stat] += count_steps
              counter[0] = out_of_bounds
              counter[1] += 1
            else
              b.steps[stat] = ideal_steps # b.steps[stat] -= counter[0]
...
class PokeBattle_Battler
  ...
  def skip_tick_down?(stat, increment)
    return false
  end
  ...
  def reverse_step_counter(stat = nil)
    if stat
      # to-do
    else
      step_counter = tracker_get(:step_counter)
