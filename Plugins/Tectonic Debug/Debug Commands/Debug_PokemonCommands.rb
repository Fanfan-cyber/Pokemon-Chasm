...
module PokemonDebugMenuCommands
      ...
      if pkmn.egg?
        ...
        loop do
          ...
          when 0   # Cure
            ...
            set_status(pkmn, ids, cmd, screen, pkmnid)
          end
        end
      end
      next false
  ...
  def set_status(pkmn, ids, cmd, screen, pkmnid)
    # Choose count
    count = 0
    params = ChooseNumberParams.new
    params.setDefaultValue(pkmn.statusCount)
    if ids[cmd] == :SLEEP
      params.setRange(1, 2)
      count = pbMessageChooseNumber(_INTL("Set the Pokémon's status further?"), params) { screen.pbUpdate }
    elsif %i[BURN NUMB FROSTBITE].include?(ids[cmd])
      params.setRange(0, 1)
      count = pbMessageChooseNumber(_INTL("Set the Pokémon's status further?"), params) { screen.pbUpdate }
    end
    pkmn.status      = ids[cmd]
